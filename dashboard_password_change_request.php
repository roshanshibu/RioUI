...
<script language="JavaScript">
  ...
  $(window).on('load', function() {
      var old_button = localStorage.getItem("cur_store_key_pcr");
  ...
  var temp = localStorage.getItem("cur_store_key_pcr");
  if(document.getElementById(temp) === null )
    {
      //alert("temp ="+temp+" inside");
      document.getElementById("curent_button_id").innerText = clicked_id;
      localStorage.setItem("cur_store_key_pcr", clicked_id);
    }
  ...
  function TrainFunc2(){
    var utterance = document.getElementById("mailtext").innerText;
    var intent =  "password_change_request";
    $.ajax({
            type : "POST",  //type of method
            url  : "addTrainingData.php",  //your page
            data : { "utterance" : utterance, "intent" : intent },// passing the values
...
$result = mysqli_query($con,"SELECT * FROM data  WHERE intent =\"password_change_request\" ORDER BY time DESC;");
...
echo ' <div class="split right scroller">
  ...
  while ($row=mysqli_fetch_array($result)) {
      ...
      if($row[0]!="password_change_request"){
