...
<script language="JavaScript">
  ...
  $(window).on('load', function() {
      var old_button = localStorage.getItem("cur_store_key_ca");
      //alert("page finished loading");
      //alert("after load"+old_button);
      document.getElementById(old_button).click();
  });
  ...
  var temp = localStorage.getItem("cur_store_key_ca");
  if(document.getElementById(temp) === null )
    {
      //alert("temp ="+temp+" inside");
      document.getElementById("curent_button_id").innerText = clicked_id;
      localStorage.setItem("cur_store_key_ca", clicked_id);
    }
  ...
  var old_button = localStorage.getItem("cur_store_key_ca");
  var element_old = document.getElementById(old_button);
...
$result = mysqli_query($con,"SELECT * FROM data  WHERE intent =\"change_address\" ORDER BY time DESC;");
while ($row=mysqli_fetch_array($result)) {
      ...
      if(!strcmp($row[3],"change_address")){
        echo '<br><input type="button" class="intentx" value="'.$row[4].'"><br>';
      }
